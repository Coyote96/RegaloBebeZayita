const elements = {
  ...
  const updateResponsiveScale = () => {
    ...
    const safeTop = Number.parseFloat(getComputedStyle(document.documentElement).getPropertyValue('env(safe-area-inset-top)')) || 0
    ...
    elements.gameStage.style.setProperty('--stage-top', '50%')

    const baseWidth = elements.gameStage.offsetWidth || elements.gameStage.getBoundingClientRect().width || 1
    const baseHeight = elements.gameStage.offsetHeight || elements.gameStage.getBoundingClientRect().height || 1

    const horizontalPadding = isPortraitMobile ? 18 : isMobileViewport ? 24 : 40
    const verticalPadding = isPortraitMobile ? 24 : isMobileViewport ? 28 : 40

    const availableWidth = Math.max(220, viewportWidth - horizontalPadding)
    const availableHeight = Math.max(320, viewportHeight - verticalPadding - safeTop - safeBottom)
    const widthScale = availableWidth / baseWidth
    const heightScale = availableHeight / baseHeight
    const nextScale = Math.min(1, widthScale, heightScale)

    elements.gameStage.style.setProperty('--game-scale', nextScale.toFixed(4))

    const scaledHeight = baseHeight * nextScale
    const centeredTopPx = Math.max(
      safeTop + 10 + scaledHeight / 2,
      (viewportHeight - safeBottom + safeTop) / 2
    )

    elements.gameStage.style.setProperty('--stage-top', `${centeredTopPx}px`)
  ...
  window.addEventListener('pageshow', () => {
    setTimeout(updateResponsiveScale, 0)
    setTimeout(updateResponsiveScale, 120)
  })
