const elements = {
  ...
  const keepGameCenteredAfterReload = () => {
    if ('scrollRestoration' in history) history.scrollRestoration = 'manual'
    forceViewportTop()
  ...
  const updateResponsiveScale = () => {
    ...
    const verticalPadding = isPortraitMobile ? 64 : isMobileViewport ? 36 : 24
    ...
    const nextScale = Math.max(isPortraitMobile ? 0.5 : 0.58, fittedScale * (isPortraitMobile ? 0.94 : 0.98))
  ...
  window.addEventListener('resize', keepGameCenteredAfterReload)
  window.addEventListener('orientationchange', keepGameCenteredAfterReload)
  window.addEventListener('load', keepGameCenteredAfterReload)
  window.addEventListener('pageshow', keepGameCenteredAfterReload)
  document.addEventListener('DOMContentLoaded', () => {
    keepGameCenteredAfterReload()
    setTimeout(keepGameCenteredAfterReload, 50)
    setTimeout(keepGameCenteredAfterReload, 250)
  })
